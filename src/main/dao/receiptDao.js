...
function findAllSortByDateAsc () {
    return new Promise((resolve, reject) => { 
        db.receipts.find({}).sort({ 'date': 1 }).exec((err, docs) => {
            if (err) {
                reject(err)
            }
            resolve(docs)
...
function findBetweenDates (startDate, endDate) {
    return new Promise((resolve, reject) => { 
        db.receipts.find({ 'date': { '$gte': startDate, '$lt': endDate } }, (err, docs) => {
            if (err) {
                reject(err)
            }
            resolve(docs)
...
function findBetweenDatesSortAsc (startDate, endDate) {
    return new Promise((resolve, reject) => { 
        db.receipts.find({ 'date': { '$gte': startDate, '$lt': endDate } }).sort({ 'date': 1 }).exec((err, docs) => {
            if (err) {
                reject(err)
            }
            resolve(docs)
